...
import { subscribeToQuestionPush, clearPushedQuestion, getPushedQuestion, type PushedQuestion } from "../utils/questionEvents";
import { supabase, REALTIME_CHANNEL } from "../utils/supabaseClient";
import { db } from "../utils/database";
import { subscribeToMeetingsUpdates, getMeetingsFromStorage, subscribeToFeaturedSpeaker, getFeaturedSpeaker } from "../utils/meetingEvents";
...
export function DashboardScreen({ name, onQuestionClick, onNotificationClick }: DashboardScreenProps) {
  ...
  useEffect(() => {
    console.log("Guest: Setting up Supabase Realtime subscription");

    // Subscribe to Supabase Realtime channel for cross-device notifications
    const channel = supabase.channel(REALTIME_CHANNEL);

    channel
      .on('broadcast', { event: 'question-push' }, (payload) => {
        console.log("Guest: Received question via Supabase Realtime:", payload.payload);
        const question = payload.payload as PushedQuestion;

        setCurrentPushedQuestion(question);
        setShowLiveQuestion(true);

        // Add to unanswered questions if not already there
        setUnansweredQuestions(prev => {
          const exists = prev.some(q => q.id === question.id);
          if (!exists && !answeredQuestions.includes(question.id)) {
      ...
      .subscribe((status) => {
        console.log("Guest: Realtime subscription status:", status);
      });

    // Fallback: Also subscribe to localStorage for same-device testing
    const unsubscribe = subscribeToQuestionPush((question) => {
      console.log("Guest: Received pushed question via localStorage:", question);
    ...
    return () => {
      console.log("Guest: Unsubscribing from question push");
  ...
  const handleAnswerQuestion = (questionId: number) => {
    // Mark as answered and remove from unanswered list
  ...
  useEffect(() => {
    ...
    const unsubscribeMeetings = subscribeToMeetingsUpdates(() => {
      loadAgenda();
    });

    const unsubscribeSpeaker = subscribeToFeaturedSpeaker(() => {
      loadFeaturedSpeaker();
    });
  ...
  const loadFeaturedSpeaker = () => {
    const speaker = getFeaturedSpeaker();
    if (speaker) {
      setFeaturedSpeaker(speaker);
    } else if (!featuredSpeaker) {
      setFeaturedSpeaker({
