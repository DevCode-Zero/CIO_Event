...
import { getPushedQuestion, type PushedQuestion, subscribeToQuestionPush } from "../utils/questionEvents";
...
export function QuestionScreen({ name, onBack }: QuestionScreenProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToQuestionPush((pq) => {
      setQuestion(pq);
    });
  ...
  const handleSubmit = async () => {
    if (question?.type === "multiple-choice" && selectedAnswer !== null) {
      try {
        await db.addResponse(question.id.toString(), "attendee", selectedAnswer, undefined, name);
      } catch (err) {
      ...
      try {
        await db.addResponse(question.id.toString(), "attendee", undefined, textResponse, name);
      } catch (err) {
        console.error("Failed to save response:", err);
      }
      setSubmitted(true);
